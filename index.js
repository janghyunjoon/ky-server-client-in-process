...
app.put("/posts/:id", (req, res) => {
    try {
        ...
        const index = findIndexId(postId)
        if (index === -1) {
            res.status(404).json({ message: "포스트 수정 중 오류" })
        }
        ...
        res.status(200).json({ message: "포스트 1개 수정 완료", post: posts[index] })
    } catch (error) {
        console.error("포스트 1개 수정 중 오류", error)
...
app.patch("/posts/:id/status", (req, res) => {
    try {
        ...
        res.status(200).json({ message: "포스트 1개 상태변경 완료", post: posts[index] })
    } catch (error) {
        console.error("포스트 1개 상태변경 중 오류", error)
        res.status(500).json({ message: "서버 오류" })
    }
})
>>>>>>> Stashed changes
